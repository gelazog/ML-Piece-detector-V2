...
// Busca una etiqueta cuyo texto contenga lo pedido.
QLabel* labelContaining(QWidget& widget, const QString& text) {
...
TEST(VariantWizard, TheVariantModeSaysItIsNotANewPiece) {
    ...
    ASSERT_NE(labelContaining(variant, QStringLiteral("Nombre del acabado")), nullptr)
        << "el campo sigue pidiendo el nombre de la pieza: quien lo rellene creerá que "
           "está registrando una pieza";
    EXPECT_EQ(labelContaining(variant, QStringLiteral("Nombre de la pieza:")), nullptr);
    ...
    auto* intro = labelContaining(variant, QStringLiteral("no una pieza nueva"));
...
TEST(VariantWizard, TheOrdinaryWizardIsUnchanged) {
    ...
    EXPECT_NE(labelContaining(normal, QStringLiteral("Nombre de la pieza:")), nullptr);
    EXPECT_EQ(labelContaining(normal, QStringLiteral("no una pieza nueva")), nullptr)
